...
const { MAX_AUD_BYTES, MAX_ISS_BYTES, MAX_NONCE_BYTES, MAX_PHONE_BYTES } = require("./constants.js");
...
const createInputs = async (data, highpartPK, lowpartPK, exp, project_id) => {
  ...
  const phoneClaim = findClaimLocation(data.jwt, data.phone, MAX_PHONE_BYTES);
  ...
  await writeFile(
    `${__dirname}/../inputs/ZKLogin_solana_SMS.json`,
